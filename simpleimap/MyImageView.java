 ...
 * MyImageView.java
 *
 * Created on January 15, 2003, 11:24 PM
 */
...
public class MyImageView extends View implements ImageObserver, MouseListener, MouseMotionListener {
		  ...
		  if( cache != null )
              ...
              String src =
                (String) fElement.getAttributes().getAttribute
                   (HTML.Attribute.SRC);
              Debug.debug("before src: " + src);
              src = processSrcPath(src);
              Debug.debug("after src: " + src);
    ...
    private String processSrcPath(String src) {
      ...
      if (WinterMute.ApplicationImagePath != null) {
          
        String imagePath = WinterMute.ApplicationImagePath;
        
        val = (new File(imagePath, imageFile.getPath())).toString();
        Debug.debug("processSrcPath", val);
      }
      ...
      Debug.debug("processSrcPath2", val);
...
//if(DEBUG)System.out.println("ImageView: getPreferredSpan");
	...
	InputStream resource = ClassLoader.getSystemClassLoader().getResourceAsStream(gifFile);

        Debug.debug("makeIcon", "gifFile", gifFile);
    ...
    private static final String
        PENDING_IMAGE_SRC = "images/image-delayed.gif",  // both stolen from HotJava
        MISSING_IMAGE_SRC = "images/image-failed.gif";
